...
public:
    ...
    virtual void operator () (Processor &p) // действия для всех команд
    {
        ...
        if ( check(fl) )
        {
            ...
            {
            case 2:
                p.setIP(p.Registers[code.addr]); // косвенный переход, регистр в поле адреса
                break;
            case 3:
                p.setIP(p.getIP()+code.addr); // относительный переход
                break;
            default:
                p.setIP(code.addr); // прямой переход
                break;
            }
        } else p.setIP(p.getIP()+3);
    }
};
